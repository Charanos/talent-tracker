...
const FeedbackTabs = ({ feedback }: FeedbackTabsProps) => {
  const [activeTab, setActiveTab] = useState<"summary" | "ats" | "details">("summary");
  ...
  return (
    <div className="w-full space-y-4">
      {/* Tab Navigation */}
      <div className="bg-black/20 border border-white/10 backdrop-blur-lg rounded-xl p-1">
        <div className="grid grid-cols-3 gap-1">
          {tabs.map((tab) => (
            <button
              ...
              className={cn(
                "relative flex flex-col items-center gap-1 px-4 py-3 rounded-lg transition-all duration-200",
                activeTab === tab.id
      ...
      <div className="relative">
        <div className={cn(
          ...
          activeTab === "summary" ? "opacity-100 translate-x-0" : "opacity-0 absolute inset-0 -translate-x-full pointer-events-none"
        )}>
        ...
        <div className={cn(
          "transition-all duration-300 ease-in-out",
          activeTab === "ats" ? "opacity-100 translate-x-0" : "opacity-0 absolute inset-0 translate-x-full pointer-events-none"
        )}>
          {activeTab === "ats" && (
            <ATS 
        ...
        <div className={cn(
          "transition-all duration-300 ease-in-out",
          activeTab === "details" ? "opacity-100 translate-x-0" : "opacity-0 absolute inset-0 translate-x-full pointer-events-none"
        )}>
